...
type DataPlatformSpec struct {
	...
	// RemoteSource is the URL of your SaaS Control Plane API
	RemoteSource string `json:"remoteSource"`

	// WorkspaceName is pulled from the API and used to create the Namespace
	// +optional
...
type DataPlatformStatus struct {
	...
	// LastSynced is the last time the operator polled the SaaS API
	// We use metav1.Time instead of time.Time
	// +optional
	LastSynced *metav1.Time `json:"lastSynced,omitempty"`

	// Status of the local components (Ready/Pending/Failed)
...
type DataPlatform struct {
	...
	// +optional
	Status DataPlatformStatus `json:"status,omitempty"`
}
...
func init() {
	SchemeBuilder.Register(&DataPlatform{}, &DataPlatformList{})
}
